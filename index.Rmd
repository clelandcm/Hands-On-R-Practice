...
8. [knitr](https://CRAN.R-project.org/package=knitr)
...
```{r, eval = FALSE}
?file.show()
file.show("table1.csv")
?read.table()
read.table(file.choose(), sep = ",", header = TRUE)
```

```{r}
table1 <- read.table("table1.csv", sep = ",", header = TRUE)
...
hsiteA <- read.xlsx("Health.xlsx", sheet = 1)
hsiteB <- read.xlsx("Health.xlsx", sheet = "Site B", detectDates = TRUE)
...
```{r}
bank <- read.spss("bank.sav", to.data.frame = TRUE)
...
```{r}
nhanes_dta <- read.dta("nhanes.dta")
...
```{r}
hsb2_sas <- read.sas7bdat("hsb2.sas7bdat")
...
```{r}
write.table(iris, file = "iris.csv", sep = ",", col.names = TRUE, row.names = FALSE)
```
...
```{r}
write.foreign(iris, datafile = "iris.dat", 
                    codefile = "iris-read.sps", 
                    package = "SPSS")
```

```{r, eval = FALSE}
file.show("iris-read.sps")
```
...
```{r}
write.dta(iris, file = "iris.dta", version=10)
```
...
```{r}
members <- read.xlsx("Bands.xlsx", 
                     sheet = "Members")
str(members)

instruments <- read.xlsx("Bands.xlsx", 
                         sheet = "Instruments")
str(instruments)
```
